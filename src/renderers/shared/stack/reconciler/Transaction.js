...
var TransactionImpl = {
   ...
   * 在一个安全窗口执行函数。使用this作为top level 方法导致 大量计算或者修改都需要安全检查。额外的参数阻止去绑定，在一些场景
   * 可选的参数可以避免很多情况下的绑定
   *
   * 
   * @param {function} method Member of scope to call. 作用域中的要调用的成员，
   * @param {Object} scope Scope to invoke from. 方法的作用域
  ...
  >(method: T, scope: any, a: A, b: B, c: C, d: D, e: E, f: F): G {
    ...
    try {
      ...
      // 捕获错误将会使调试更加困难，所以我们使用errorThrown 设置为true，在设置他为false之前，在调用close之后，
      // 如果在最后的块中，还是被设置为true， 这意味着这其中的一些调用抛出了异常
      errorThrown = true;
