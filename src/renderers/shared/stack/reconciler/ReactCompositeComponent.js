...
var ReactCompositeComponent = {
  ...
  construct: function(element) {
    ...
    // see ReactUpdates 和 ReactUpdateQueue
    this._pendingCallbacks = null;

    // ComponentWillUnmount shall only be called once
    // ComponentWillUnmount 只能被调用一次
   ...
   * 初始化component, render markup, 注册事件监听
   *
   * @param {ReactReconcileTransaction|ReactServerRenderingTransaction} transaction
   * @param {?object} hostParent
   * @param {?object} hostContainerInfo
   * @param {?object} context
   * @return {?string} Rendered markup to be inserted into the DOM. 渲染markup 插入到DOM
   * @final
   * @internal
   */
  mountComponent: function(
    ...
    this._hostContainerInfo = hostContainerInfo; //{info: } 对象

    var publicProps = this._currentElement.props;
    var publicContext = this._processContext(context);

    var Component = this._currentElement.type; //root TopLevelWrapper
    ...
    if (__DEV__) {
      ...
      warning(
        ...
        'Setting defaultProps as an instance property on %s is not supported and will be ignored.' +
          ...
          // 在一个component设置 defaultProps 作为一个实例属性 是不被支持的，将会被忽略。
          // 你可以定义defaultProps 作为一个静态属性在一个组件上(component)
        this.getName() || 'a component',
        this.getName() || 'a component',
      );
    }
    ...
    invariant(
      ...
      // 在React 组合组价中, state 必须是一个对象或者 null
    ...
    if (inst.componentWillMount) {
      ...
      // 当挂载时，由componentWillMount调用setState 将设置到this._pendingStateQueue上, 就不会触发重新渲染 
      // 去获取state
      // componentWillMount 里面设置state， 会先会合并到实例里面去
...
// 构造没有所属者的组件
  ...
  performInitialMount: function(
    ...
    // 调用组件render()方法后的结果 一个element元素对象
   ...
   * 如果 _pendingElement, _pendingStateQueue or _pendingForceUpdate 中的任意一个设置了，
   * 就要更新组件.
   *
   ...
   * 在一个挂载组件上，执行更新。componentWillReceiveProps 和 shouldComponentUpdate 方法会被调用
   * 然后(假定更新没被忽略) 后续的生命周期更新方法会被调用，DOM会被更新
   *
   * By default, this implements React's rendering and reconciliation algorithm.
   * Sophisticated clients may wish to override this.
   * 默认情况下,这里实现了React rendering和 协调者算法.
   * 复杂的客户端可能希望覆盖这个方法
   *
   * @param {ReactReconcileTransaction} transaction
  ...
  updateComponent: function(
    ...
    // 这里的一个更新会调度一个更新，但是立即设置 _pendingStateQueue ,会立即确定任何state(状态)的更新会立即被协调(调度),
    // 而不是等到下一次批量更新。
    ...
    // 如果更新发生在入队一个更新的时候，我们不应该执行新的回调，知道下一次渲染发生时才能执行，所以先挂起回调，
    var callbacks = this._pendingCallbacks;
    this._pendingCallbacks = null;

    // 这里才会把 setState 的值进行合并，与实例的setState 进行合并
    ...
    if (shouldUpdate) {
      ...
      // 会设置(更新) this.props， this.state 和 this.context
      ...
      // 如果确定一个组件不需要更新，我们还是想要设置props 和 state, 但是我们会缩短更新路径
      // 只会更新属性
   ...
   * performs update.
   * 合并新props 和 state, 通知委托的更新方法，然后执行更新
   *
