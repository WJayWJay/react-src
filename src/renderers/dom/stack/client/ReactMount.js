...
function mountComponentIntoNode(
  ...
  var markup = ReactReconciler.mountComponent(
    ...
    transaction,
    null,
    ReactDOMContainerInfo(wrapperInstance, container), //{info: } 对象
    context,
    0 /* parentDebugID */,
  );
...
var ReactMount = {
  ...
  _renderNewRootComponent: function(
    ...
    // 初始化render 是同步的，但是任何发生在渲染期间的更新，在componentWillMount or componentDidMount 都会根据当前批处理策略
    // 将会被批处理
  ...
  _renderSubtreeIntoContainer: function(
    ...
    var nextContext = getContextForSubtree(parentComponent); //null
    var prevComponent = getTopLevelWrapperInContainer(container); //null 首次为null
    ...
    var reactRootElement = getReactRootElementInContainer(container); //首次为undefined
    var containerHasReactMarkup =
      reactRootElement && !!internalGetID(reactRootElement); //首次为undefined false
    var containerHasNonRootReactChild = hasNonRootReactChild(container); //undefined
   ...
   * perform an update on it and only mutate the DOM as necessary to reflect the
   * latest React component.
   * 如果React 组件先前已经渲染到container(容器), 这里会在它上面执行更新，只会修改需要更的dom，反射到React 组件上
   *
   * @param {ReactElement} nextElement Component element to render.
   * @param {DOMElement} container DOM element to render into.
   * @param {?function} callback function triggered on completion
